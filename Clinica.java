...
public class Clinica {
	...
	private Set<Perro> setPerros;

	public Clinica() {
		this.mapaConsultas = new LinkedHashMap<Animal, Set<Consulta>>(); //Por orden de inserccion
		this.setPerros = new TreeSet<Perro>();
	}

	public void addConsulta( Consulta consulta) {
	      
		Animal clave= consulta.getIdAnimal(); //La clave del mapa es un Animal
		...
		//Si es un perro, lo añadimos al set de Perros. Esto es para despues sacrlos ordenados por raza
		if  (clave instanceof Perro) {
			 setPerros.add((Perro) clave);
		}
		System.out.println("Añadida consulta para "+consulta.getIdAnimal());
	}
	...
	public void aFichero() {//Vuelca en un fichero las claves del mapa, esto es, los Animales
        ...
        try {
            ...
            Iterator  itr = mapaConsultas.entrySet().iterator();  //EntrySet devuelve todo del mapa
